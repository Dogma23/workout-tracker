...
- **Supersets** — tap **Link as superset** between two (or more) consecutive exercises to pair them — in a day editor (saved for every future session) *or* on the fly during a workout (that session only). Grouped moves render as one linked block and you log each move's sets on its own card — but the rest timer stays quiet between them and only fires after the last move in the group, so you go straight from one exercise into the next. Tap the connector again to unlink.
